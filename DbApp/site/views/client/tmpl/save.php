...
//JHtml::_('behavior.formvalidation');
  ...
  if ($searchid == 0) {
    $newquery = " INSERT INTO #__aaaclient ( ";
    $columns = "";
    $vcolumn = "";
  ...
  foreach ($afteredit as $key => $value) {
    ...
    if (($key == 'principalinvestigator') || ($key == 'catid') || ($key == 'department') || ($key == 'address') || ($key == 'vatno') || ($key == 'comment') || ($key == 'user') || ($key == 'time')) {
      $columns .= $key . ", ";
...
//    $aearr = array($afteredit);
  //  $db->updateObject('#__aaaclient', &$aearr, 'id');
    ...
    $itemid = $menu->id;
    echo "<br /><a href=index.php?option=com_dbapp&view=clients&Itemid=" . $itemid . ">Return to client list</a><br />&nbsp;<br />";
?>
