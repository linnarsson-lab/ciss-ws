...
//JHtml::_('behavior.formvalidation');
  ...
  if ($searchid == 0) {
    $newquery = " INSERT INTO #__aaasequencingprimer ( ";
    $columns = "";
    $vcolumn = "";

    $query = " UPDATE #__aaasequencingprimer SET ";
    
  echo "<table>";
  foreach ($afteredit as $key => $value) {
    echo "<tr><td>" . $key . "</td><td>" . $value . "</td></tr>";
    if (($key == 'primername') || ($key == 'sequence') || ($key == 'comment') || ($key == 'user') || ($key == 'time')) {
      $columns .= $key . ", ";
...
//    $aearr = array($afteredit);
  //  $db->updateObject('#__aaaclient', &$aearr, 'id');
    ...
    $itemid = $menu->id;
    echo "<br />&nbsp;<a href=index.php?option=com_dbapp&view=sequencingprimers&Itemid=" . $itemid . ">Return to sequencing primers list</a><br />&nbsp;<br />";
?>
